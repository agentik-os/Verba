...
*Last updated: 2026-06-12 — integrates the JARVIS wave (voice → action via Composio, on-device action planning), stacked dictations, i18n (14 languages), and the rebuilt /compare page (commits 5ae8804…ebcdcff).*
...
## Product Overview
**One-liner:** Speak it. Send it clean. — the most complete voice-to-text on the Mac, and the only one that **acts** on what you say.
...
**New (June 2026): voice → action.** Action mode is now **JARVIS**, a voice agent: say *"create a Linear issue for
this bug"* or *"email Anna the summary,"* and Verba plans the steps, asks a quick question when something is ambiguous,
shows you exactly what it will do, and executes **only after you confirm** — natively on the Mac (Calendar, Reminders,
Shortcuts, Messages, AppleScript: `Sources/Verba/ActionExecutor.swift`) and across **1,000+ connected apps** (Gmail,
Slack, Notion, Linear, GitHub…) via a secure Composio-backed relay with a fail-safe read/write classifier
(`website/lib/composio-rw.ts`; writes are never auto-run — commits 5ae8804, 7452012). The **action plan is generated
on-device** by the user's own Claude Code / local model / own key — never a server key (commits d86685b, b428a1f).
**Product category (the "shelf"):** AI voice dictation / voice-to-text for Mac — how people search: *"Mac dictation app,"
"Wispr Flow alternative," "local dictation Mac," "voice to text for coding."* **Emerging second shelf Verba now also
owns:** the desktop **voice agent** — *"Jarvis for Mac," "control apps by voice," "voice assistant that actually does things."*
...
**(a) vibe coding** (a 20-minute voice ramble → a tidy spec/prompt for Claude Code or Cursor), **(b) messaging/writing**
(Slack, Mail, Notion, docs) in your own voice, and now **(c) voice → action** (say the intent — "create the issue,"
"send the invite" — confirm, done).
**Jobs to be done:**
...
- "When I say 'schedule it / send it / file it,' **just do it — after showing me exactly what you'll do.**"
...
narrating a bug report into a clean ticket; journaling an hour of thoughts into an outline; dictating "create the Linear
issue and email the team the summary" and confirming both actions from the JARVIS feed; chaining a string of thoughts as
**stacked dictations** (start #2 while #1 is still processing — each lands when ready, commit e104b79).
...
| **The Voice-first operator** *(new, opened by JARVIS)* — founder/PM/EA-less exec living in Gmail, Slack, Linear, Calendar | Fewer context switches, speed, control over what an agent does | Every spoken intent still means opening an app, clicking through UI; existing "assistants" can't touch their real tools | **Say it once** — JARVIS plans the action, shows it, you confirm; it executes across **1,000+ connected apps** + native Mac actions, with the plan generated by **your own AI, on-device** |
...
- And **none of them can *act* on what you say** — at best TalkTastic offers app-aware commands on the Mac itself; no
  dictation competitor executes confirmed actions across 1,000+ connected apps (`website/lib/competitors.ts`, `compare-matrix.ts`).
...
3. **Voice → action (JARVIS)** *(category-of-one; verified `ActionExecutor.swift`, `ActionAgentClient.swift`, commits 5ae8804, 7452012)* — the only dictation app that **executes**: dictate the intent, JARVIS plans the steps (asking when ambiguous, filling missing fields), shows the action, and acts only on your confirm — across **1,000+ connected apps** plus native Mac actions. The **plan is generated on-device by your own AI** (Claude Code sub / local model / own key — never a server key, d86685b, sandboxed b428a1f). Reliability is engineered, not hoped for: every proposed action is validated against the tool's **real JSON schema and auto-repaired** (0bec783, 83da81b — full catalog hardened: 989 toolkits / 36,998 tools); the relay's fail-safe classifier treats anything ambiguous as a write and **never auto-runs writes** (`website/lib/composio-rw.ts`).
4. **Does what others can't** — **Context mode** (reads your screen with vision, grounds the output) + **hour-long structured Notes** + **live Translate**. Most rivals only transcribe.
5. **Six modes, the right model each** — Flow (no AI), Polish, Intent, Translate, Context, Coding (Opus) — routed to Haiku/Sonnet/Opus so you pay for power only where it matters. Edit any prompt or build your own custom mode. Modes are now **sticky** (explicit switches persist) and instantly reachable via the **Fn+digit chord** (b428a1f); **stacked in-flight dictations** let you chain recordings without waiting (e104b79).
6. **It learns you** — auto-learns vocabulary from your edits, matches your writing tone per app, formats per app (markdown vs plain).
7. **Cheaper and honest** — $9.99/mo vs $12–17 for the cloud incumbents; the `/vs` pages and the rebuilt `/compare` matrix (24 features × 10 brands, `website/lib/compare-matrix.ts`) openly state where rivals still win.
**How we do it differently:** the AI-restructuring layer (Claude + modes) is the *core*, not an add-on; the entire
pipeline — including the **action planner** — can run on the user's own hardware and their own AI account.
**Why customers choose us:** they get *more* (actions, vision, notes, translate) for *less* ($9.99), keep their data on
...
| "Why not Wispr Flow?" | It uploads your audio, costs $15, and locks you into its AI markup. Verba is on-device, $9.99, and lets you bring the AI you already pay for. And Wispr types; Verba can also **do** — create the event, send the email, file the issue. |
| "A voice agent with access to my Gmail/Slack/Linear? No thanks." | Built paranoid by design: the agent may only auto-run **read-only** steps (capped, version-resolved); **every write is shown to you and executes only on your confirm** — the classifier fail-safes anything ambiguous to "write" (`composio-rw.ts`). The *plan* is generated **on your Mac by your own AI** — the relay holds no model key (d86685b). Disconnect any app anytime in Settings ▸ Connected apps. |
...
- "I just say it and it creates the issue / sends the email — after I confirm."
- "Jarvis for my Mac, finally."
**Words to use:** on-device, private, your AI, your Claude sub, no markup, bring your own, clean, modes, offline, Apple-Silicon, vibe coding, one press, **voice agent, JARVIS, connected apps, it asks before it acts, confirm**.
**Words to avoid:** "uploads to our servers," "cloud-only," "we train on your data," "unlimited cloud minutes," enterprise jargon, "transcription" used alone (Verba is *restructuring*, not just transcription). **"Composio" in public copy** — the user-facing names are *JARVIS* and *connected apps*; all Composio mentions were deliberately removed from the app + site (commit 5ae8804). Composio stays in internal docs only.
**Glossary:**
| Term | Meaning |
|---|---|
...
| **JARVIS / Action mode** | Verba's voice agent: dictated intent → on-device plan → clarify/fill-in → **confirm** → execute (native Mac actions + connected apps) |
| **Connected apps** | The 1,000+ third-party apps (Gmail, Slack, Notion, Linear, GitHub…) JARVIS can act on — connected per-app in Settings via OAuth or API key (adaptive connect), Composio-backed under the hood |
| **On-device planning** | The action plan is generated on the Mac by the user's own engine (Claude Code sub / local Ollama / own key) — never by a server-held model key |
...
- **JARVIS action layer:** **1,000+ connected app toolkits**; full-catalog hardening verified at **989 toolkits / 36,998 tools, 100% schema coverage** (83da81b); every proposed action schema-validated + auto-repaired (0bec783); certified live with adversarial read-backs — **writes 5/5** (Calendar create+delete, Gmail draft, GitHub star/unstar, Linear comment, Perplexity) (b428a1f).
- **On-device action planning** — the planner runs sandboxed on the user's Mac (their Claude Code / local model / own key), never the server key; verified live with the server key exhausted (d86685b).
- **UI localized: 998 strings translated into 14 languages** (b3dd02c) — the app is international-ready, not en-only.
- **$9.99/mo vs $12–17** cloud incumbents; **zero inference COGS** (BYOK — including the action planner).
...
| Theme | Proof |
|---|---|
| Private by default | On-device Whisper/Parakeet; audio never uploaded; local history off-switch; keys in Keychain; **even the action planner runs on-device** |
| Bring your own AI | Claude Code sub (no key) / Anthropic / OpenRouter / local Ollama — for restructuring *and* action planning |
| **Voice → action** | JARVIS: plan → clarify → confirm → execute; 1,000+ connected apps; schema-validated + auto-repaired; writes never auto-run |
| Does more than transcribe | Context (vision), Notes (hour-long), Translate, stacked dictations |
| Cheaper, honestly | $9.99 vs $12–17; transparent /vs pages + the 24-feature × 10-brand /compare matrix |
...
MRR/user numbers yet. **Two known fixes before scaling spend** (both grounded, both ~1-day; **re-verified 2026-06-12, both still open**): (1) the site now standardises on
**33 dictations** (hero, bento, /compare, /vs — matching `Entitlement.swift` `freeTrialDictations = 33`), but the web "Try-It" demo
endpoint (`website/app/api/try/route.ts` lines 56/67) still nudges "free up to 10,000 words/month" — align that copy. (2) **privacy-claim accuracy:** audio is kept in local history by default (`History.swift`; `Settings.swift` `saveHistory ?? true`) — never *uploaded* (sync is text-only) but stored locally, so the claim must stay "audio never leaves your Mac / off switch," never "nothing written to disk" (or default history off).
