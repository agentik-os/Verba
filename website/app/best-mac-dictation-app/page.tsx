...
const faq = [
  ...
  { q: "Does Verba work offline?", a: "Yes. In on-device mode Verba transcribes locally with WhisperKit or NVIDIA Parakeet, so dictation works with no internet and your audio never leaves your Mac. You can also run the AI cleanup offline with a fully local model. A connection is only needed for hosted-AI features like cloud rewriting, live translation, or the JARVIS agent." },
  { q: "How does Verba compare to superwhisper on price?", a: "Verba is $9.99/mo or $84/year, and you bring your own AI key or run a local model, so there is no token markup. Raw dictation is free forever and unlimited with no card. Both apps also offer a one-time lifetime: Verba's Founder's Edition is $149, superwhisper's is $249.99. Verba is the better pick if you also want AI cleanup in any app, live translation, and a voice agent." },
  { q: "Which Mac dictation app has a voice agent?", a: "Verba is the only Mac dictation app with a built-in voice agent: JARVIS (Action mode) plans the steps, asks to clarify, and, after you confirm, acts across 1,000+ connected apps (Gmail, Slack, Notion, Google Calendar, Linear, GitHub and more), always after you confirm. Other apps only turn speech into text." },
];
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 pb-24">
      ...
      <article className="py-8">
        ...
        <p className="mt-5 text-lg leading-relaxed muted">{`We tested every serious Mac voice-to-text app against the things that actually matter in 2026: accuracy, whether your audio leaves your machine, how good the AI cleanup is, and what you can do with your voice beyond typing. Verba comes out on top for most people because it does the rare trifecta of on-device transcription, bring-your-own-AI, and a voice agent that acts on your connected apps, all for $9.99/mo. But the right pick depends on your priorities, so here is the honest, sourced breakdown.`}</p>
        ...
        <section className="mt-10">
          ...
          <p className="mt-3 leading-relaxed muted">{`Verba is the only app here that runs on-device by default (WhisperKit / Parakeet, audio never leaves your Mac), lets you bring your own AI, and ships a real voice agent. BYO-AI is the standout: route cleanup to your Claude plan with no API key, or an Anthropic / OpenAI / OpenRouter key, or a fully local model, set per mode. JARVIS, its voice agent, lets you speak an intent and have it executed across 1,000+ connected apps, with the keys held by a secure relay and never stored on your Mac. Raw dictation is free forever and unlimited (no card); Pro is $9.99/mo (or $84/yr), or a one-time $149 lifetime, undercutting every cloud competitor. The honest caveat: it's Apple Silicon macOS only, no Windows, iOS or Android yet.`}</p>
        </section>
        <section className="mt-10">
        ...
        <section className="mt-10">
          ...
          <p className="mt-3 leading-relaxed muted">{`superwhisper is the most mature on-device-first competitor, with a well-loved local stack (Whisper + Parakeet), custom prompt modes, bring-your-own cloud LLMs, and Mac / Windows / iOS coverage. It also offers a $249.99 lifetime license if you'd rather not subscribe (Pro is roughly $8.49 to $8.99 a month). The catches: it writes your audio recordings to disk by default with no easy opt-out, and stores API keys in plaintext. If you want local transcription with a longer track record of model management it's the strongest alternative to Verba, though Verba matches it with its own $149 lifetime option and never writes your audio to disk.`}</p>
        </section>
        <section className="mt-10">
          <h2 className="text-2xl font-semibold tracking-tight">{`MacWhisper, best for transcribing files`}</h2>
          <p className="mt-3 leading-relaxed muted">{`MacWhisper is excellent at what it's built for: batch-transcribing existing audio and video files, fully locally via whisper.cpp, with strong privacy and a one-time purchase (€59 / $99 lifetime on Gumroad, or an App Store subscription from $6.99/mo). It's a transcription utility first, not a type-anywhere dictation tool, AI cleanup is a secondary add-on rather than the core, and it isn't tuned for the speak-then-paste-at-your-cursor workflow. Verba also transcribes dropped audio and video files, so the real split is specialization: pick MacWhisper if your main job is bulk-converting recordings, pick Verba if you want live dictation, agent actions, and file transcription in one app.`}</p>
        </section>
        <section className="mt-10">
          <h2 className="text-2xl font-semibold tracking-tight">{`Honorable mentions, Aqua Voice, VoiceInk, Apple Dictation`}</h2>
          <p className="mt-3 leading-relaxed muted">{`Aqua Voice has genuinely sharp natural-language editing ("make a list", "rephrase") powered by its proprietary Avalon model, but it's cloud-only with a tiny 1,000-word free cap, so your audio always leaves your device. VoiceInk is open-source (GPLv3), fully local, and a one-time $25 to $49 purchase, great for privacy purists willing to bring their own AI key and forgo a managed account. Apple Dictation is free, built in, and on-device for many languages on Apple Silicon, but offers no AI cleanup at all. You get a raw transcript and fix the punctuation and structure yourself.`}</p>
        </section>
        <section className="mt-10">
          <h2 className="text-2xl font-semibold tracking-tight">{`How we picked`}</h2>
          <p className="mt-3 leading-relaxed muted">{`We rated each app on the same axes our /compare matrix tracks: on-device transcription, AI cleanup quality and control, type-anywhere auto-paste, voice-agent capability, privacy defaults, platform coverage and price. Every claim above is drawn from each vendor's own site, pricing page and public reviews, verified mid-2026, nothing inflated, and we note where each rival beats Verba. Verba leads on the on-device + BYO-AI + voice-agent combination specifically; if your priority is mobile apps or the deepest local-model track record, the rankings above point you to the better fit.`}</p>
        </section>
        <section className="mt-10 glass rounded-2xl p-6">
          <h2 className="text-xl font-semibold tracking-tight">The verdict</h2>
          <p className="mt-3 leading-relaxed muted">{`For most Mac users in 2026, Verba is the best dictation app: it's the only one that combines on-device transcription (your audio never leaves the Mac), bring-your-own-AI cleanup you actually control, your Claude plan with no key, any Anthropic / OpenAI / OpenRouter key, or a fully local model, and JARVIS, a voice agent that acts across 1,000+ connected apps, with raw dictation free forever and unlimited (no card), Pro at $9.99/mo, and a one-time $149 lifetime. If you need Windows or mobile, Wispr Flow is the polished cloud pick; if you want the longest local track record, superwhisper is the closest alternative; and if your main job is bulk-converting existing audio and video files, MacWhisper is purpose-built for that. But if you want the most capable, private, and flexible everyday dictation on a Mac, Verba is where we'd start.`}</p>
          ...
          <span className="ml-3 text-xs muted">Unlimited raw dictation free forever · Pro $9.99/mo</span>
        </section>
