...
> Date: 2026-06-11 · Backing detail: `market-research.md` · `../.agents/product-marketing.md` · `gtm-strategy.md` · `content-strategy.md`

## The product, in one line
**Verba turns your rambling speech into clean, ship-ready text anywhere on your Mac** — press a key, talk, and Claude
restructures it where your cursor is. *Speak it. Send it clean.* ($9.99/mo · macOS · live at verba.run)
...
AI restructuring + bring-your-own-Claude." That empty slot is Verba's.** Three differentiators, two of them category-of-one:
1. **Bring your own AI — including your Claude Code sub, with no key.** *No competitor has this.* Zero inference cost to us.
2. **Private by default** — on-device transcription, **your audio never leaves the Mac and is never uploaded** (cloud sync is
   text-only; local history has an off switch). Beats every cloud tool, which uploads every word; vs Superwhisper the edge is
   Keychain-stored keys + an off switch (both keep local history by default).
3. **Does what others can't** — reads your screen (Context/vision), hour-long structured Notes, live Translate, agentic
   Calendar/Reminders/email actions. Most rivals only transcribe.
Priced **below** the cloud incumbents ($9.99 vs $12–17) while doing more — and at a **very high gross margin** (users bring the AI, so zero inference cost).
...
Cursor/Claude Discords, dev YouTube; reachable with **zero ad spend**). Then expand to privacy-first professionals →
multilingual workers → long-form note-takers. Three compounding engines:
- **Comparison & use-case SEO** (the `/compare` + `/vs` pages already ship — high-intent, bottom-funnel).
- **Community & launch** (Show HN, Product Hunt, founder building-in-public — the product is inherently demoable).
- **Creators** (dev/Mac reviewers; "speak-vs-type, and it reuses your Claude sub" sells itself).
...
1. **Fix two residual copy gaps (both ~1-day, high leverage):** the site now says "33 dictations" everywhere except the web
...
2. **Nail first-run activation** (zero-key Claude Code path + instant offline Parakeet; first dictation < 60s) and instrument the funnel.
3. **Launch to developers** — comparison + "voice for Claude Code" content, Show HN, seed 10–20 creators, ship the 60-sec demo, add a Lifetime/Founder tier as the launch hook.

## Why this is a good co-founder bet
A **real, shipped product** in a **$2B-validated, fast-growing category**, with **two category-of-one moats**, a **very high gross
margin** (zero inference COGS), and **growth loops already built** (referral, leaderboard, full gamification) — run by a founder who builds agentic systems and markets in public. The product
