...
> **AMENDEMENT v6 (2026-07-03, opérateur) — DESIGN = LE VRAI SITE** : « le même design que le site internet ». Les blueprints IA sont abandonnés. Méthode : capturer verba.run + ses composants (Playwright) et rendre les scènes en HTML avec les tokens exacts du site (`website/app/globals.css` : t-display, eyebrow, panel ; Inter) → screenshot 1080×1920. Composants réels réutilisés (panneau Context « Pasting into Mail », carte JARVIS « Confirm · Gmail »). Puis même pipeline (voix off + captions calées + push-in + musique + end card). Test `verba-site-promo.mp4` (22s) envoyé. Assets : `Station/Marketing/projects/verba/assets/video/tech/site-scenes/`.

> **AMENDEMENT v5 (2026-07-03, opérateur)** : bascule vers le **contenu technique graphique** (animé + voix off), l'UGC talking-head est déprécié comme défaut. Chaîne : designs blueprint (validés) → voix off ElevenLabs → captions calées WORD-LEVEL sur la voix, découpées sur les pauses réelles + durée mini 0.9s (fix « sous-titres trop rapides ») → push-in lent ffmpeg (jamais d'image-to-video sur du texte) → nappe duckée → end card. Template + notes : `Station/Marketing/projects/verba/assets/video/tech/`. Premier test `verba-tech-promo.mp4` (32s) envoyé.
